...
abstract class AbstractNeumorphicEmbossPainterCache {
  ...
  bool updateSize({required Offset newOffset, required Size newSize}) {
    if (_cacheOffset != newOffset ||
      ...
      final double middleWidth = newSize.width / 2;
      final double middleHeight = newSize.height / 2;
  ...
  void updatePath({required Path newPath}) {
    _path = newPath;
    subPaths.clear();
    final pathMetrics = newPath.computeMetrics();
    for (final PathMetric item in pathMetrics) {
